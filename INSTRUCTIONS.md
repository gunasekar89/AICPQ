...
The diagram below summarizes how the modular and extensible agents exchange data across classical and quantum paths, highlighting the Azure Quantum integration points and performance benchmarking loop that power the demo experience.

```mermaid
flowchart TD
    UI[Streamlit Dashboard / CLI Requests]
    Planner[Planner Agent\nGemini Intent Parsing]
    Pref[Preference Agent\nQLSTM / User Profiles]
    Vision[Vision Agent\nCLIP + QCNN]
    QOI[Quantum Optimization & Insight Agent\nAzure Quantum + Simulators]
    Synth[Synthesis Agent\nVQE Multi-Objective Optimizer]
    Brand[Brand Agent\nEco Ranking & Scoring]
    Fulfill[Fulfillment Agent\nQuantum Annealing Routes]
    DB[(SQLite Demo Database)]
    Products[(Demo Products & Images)]
